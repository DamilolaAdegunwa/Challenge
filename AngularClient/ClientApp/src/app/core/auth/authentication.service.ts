...
import { Credential, ResetPasswordInput } from './credential';
...
export class AuthenticationService implements AuthService {
	...
	public login(credential: Credential): Observable<any> {
		...
		return this.http.post(environment.apiUrl + '/api/Auth/login', JSON.stringify({ username: credential.email, password: credential.password }), { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) })
			.pipe(

			//return this.http.post<AccessData>(environment.apiUrl + '/api/Auth/login' + '?' + this.util.urlParam(credential)).pipe(
			map((result: any) => {
	...
	private saveAccessData(accessData: AccessData) {
		if (typeof accessData !== 'undefined') {
			...
			this.tokenStorage
				...
				.setRefreshToken(accessData.expires)
				//.setUserRoles(accessData.roles);
				//.setUserRoles(accessData.roles)
				.setUserRoles(accessData.roles)
				.setUserData(accessData.displayName, accessData.email)
	 ...
	 * @returns {Observable<any>}
	 */
	public requestPassword(credential: ResetPasswordInput): Observable<any> {
		return this.http.post(environment.apiUrl + '/api/Auth/ResetPassword', credential)
			.pipe(
